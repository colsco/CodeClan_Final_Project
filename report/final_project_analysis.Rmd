...
model_18_annual <- model_18 %>% 
  ...
  select(-c(industry_group, region)) %>% 
  group_by(year) %>%
  summarise(total_predicted = sum(avg_pred)) # think this is summing this twice...?

head(model_18_annual)
```
