...
and region have an influence on productivity, but the average number of jobs in 
each region is insignificant.
...
```{r}
plot(summary_mod1a_exh$bic, type = "b")
```
...
base_2_no_region <- base_uk_jobs_edu_prod %>% 
  select(-region) %>% 
...
This is a small data set, so set a 60:40 split
```{r}

# Make a test index
test_index_2 <- sample(1:nrow(base_2_no_region), 
                       size = nrow(base_2_no_region)*0.4)

# Use the test index to create test and training datasets
model2_test  <- slice(base_2_no_region, test_index_2)
model2_train <- slice(base_2_no_region, -test_index_2)
```



```{r}  
edu_mod1 <- regsubsets(pounds_per_hour_worked ~ ., 
                            data = model2_train,
                            method = "exhaustive")

regsubsets_edu_train <- summary(edu_mod1)
regsubsets_edu_train
```
...
```{r}
plot(regsubsets_edu_train, scale = "adjr2")
```
